...
export class UserService {
  ...
  async verificationAgain(authUser): Promise<VerificationAgainOutput> {
    try {
      const user = await this.users.findOne(
        {
          select:{
  ...
  async getVideoList(): Promise<VideoOutput>
    {         
        ...
        const item = 3  
            ...
            const videoList = await lastValueFrom(this.httpService.get(`https://api.wecandeo.com/info/v1/videos.json?key=${apiKey}&pkg=${pkg}&page=${pageNumber}&pagesize=${item}`));
            // console.log("비디오리스트", videoList.data.videoInfoList.videoInfoList)
            
            if(videoList) {
                const episodeList = []
                await videoList.data.videoInfoList.videoInfoList.forEach(async (key) => {
