...
export class UserService {
  ...
  async githubLogin({ code }: GitHubOAuthInput): Promise<GitHubOAuthOutput> {
    try {
      ...
      const access_json = JSON.stringify(access.data);  
      const access_temp = JSON.parse(access_json)
      const access_token = access_temp.access_token ?? ""
      ...
      console.log(user_email.data[0].email)
      return {
        ok: true,
        token: "hello"
      }
