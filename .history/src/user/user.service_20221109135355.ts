...
export class UserService {
  ...
  async githubLogin({ code }: GitHubOAuthInput): Promise<GitHubOAuthOutput> {
    try {
      ...
      const access_token = await lastValueFrom(this.httpService.get(
      ...
      const access_json = JSON.stringify(access_token.data);  
      const jss = JSON.parse(access_json)
      console.log(jss.access_token)
      // const user_data = await lastValueFrom(this.httpService.get(
      //   "https://api.github.com/user",
      //   {
      //     headers:{"Authorization":`Bearer ${access_json.access_token}`}  
      //   }
      // ))
      return {
        ok: true,
