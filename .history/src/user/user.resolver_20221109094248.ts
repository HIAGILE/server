...
import { OAuthInput, OAuthOutput } from "./dtos/oauth-login.dto";
...
export class UserResolver {
    ...
    @Mutation((returns) => OAuthOutput)
    async githubLogin(@Args('input') oAuthInput:OAuthInput):Promise<OAuthOutput>{
        return await this.userService.githubLogin(oAuthInput);
    }

    @Mutation((returns) => OAuthOutput)
    async kakaoLogin(@Args('input') oAuthInput:OAuthInput):Promise<OAuthOutput>{
        return await this.userService.kakaoLogin(oAuthInput);
    }
