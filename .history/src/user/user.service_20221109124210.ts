...
export class UserService {
  ...
  async githubLogin({ code }: GitHubOAuthInput): Promise<GitHubOAuthOutput> {
    try {
      ...
      console.log(`https://github.com/login/oauth/access_token?code=${code}&client_id=7cf6aa6be0c8c9c03214&client_secret=${process.env.GH_KEY}`)
      const access_token = await this.httpService.get(
          `https://github.com/login/oauth/access_token?code=${code}&client_id=7cf6aa6be0c8c9c03214&client_secret=${process.env.GH_KEY}`,
          { headers: headersRequest });
      
      const data = await access_token.subscribe(
        (value) => value.data
      )
