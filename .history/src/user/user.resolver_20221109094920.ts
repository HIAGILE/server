...
export class UserResolver {
    ...
    async githubLogin(@Args('input') oAuthInput:GitHubOAuthInput):Promise<GitHubOAuthOutput>{
        return await this.userService.githubLogin(oAuthInput);
    }

    @Mutation((returns) => KakaoOAuthOutput)
    async kakaoLogin(@Args('input') oAuthInput:KakaoOAuthInput):Promise<KakaoOAuthOutput>{
